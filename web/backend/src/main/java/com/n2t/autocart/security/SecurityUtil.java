...
public class SecurityUtil {
    ...
    @Value("${auto-cart.jwt.access-token-validity-in-seconds}")
    private int accessAxpiredTime;
    ...
    public String createRefreshToken(String email, LoginResponseDTO loginResponseDTO){
        Instant now = Instant.now();
        Instant validity = now.plus(this.refreshAxpiredTime, ChronoUnit.SECONDS);
        JwtClaimsSet claims = JwtClaimsSet.builder()
                .id(UUID.randomUUID().toString())
                .issuedAt(now)
                .expiresAt(validity)
                .subject(email)
                .claim("user", loginResponseDTO.getUserResponse())
                .build();
        JwsHeader jwsHeader = JwsHeader.with(JWT_ALGORITHM).build();
        return this.jwtEncoder.encode(JwtEncoderParameters.from(jwsHeader, claims)).getTokenValue();
    }
